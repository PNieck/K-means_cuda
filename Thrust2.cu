...
Thrust2Data createThrust2Data(const Points& points, const Centroids& centroids)
{
	...
	for (int i = 0; i < result.dim_cnt; i++) {
		for (int j = 0; j < result.points_cnt; j++) {
			result.points_coord[i + j * result.dim_cnt] = points.coordinates[i][j];
		}
	}
	...
	for (int i = 0; i < result.dim_cnt; i++) {
		for (int j = 0; j < result.centr_cnt; j++) {
			result.centr_coord[i + j * result.dim_cnt] = centroids.coordinates[i][j];
		}
	}
...
int find_nearest_centroids(Thrust2Data& data)
{
	...
	//swap
	thrust::copy(data.new_centr_indexes.begin(), data.new_centr_indexes.end(), data.centr_indexes.begin());
...
void KMeansAlg::thrust2_version(Points& points, Centroids& centroids, float threshold, int max_it)
{
	...
	std::cout << "GPU version 2 time: " << time << " (milliseconds)." << std::endl;
